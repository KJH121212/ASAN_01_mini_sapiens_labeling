...
class SapiensLiteDataset(Dataset):
    def __init__(self, tasks, frame_dir):
            ...
            for obj in objects:
                if obj.get('bbox'):
                    ...
                    self.items.append({
                        ...
                        'frame_idx': f_idx, 'obj_id': obj['id'], 'bbox': bbox
                    })
                    # 좌표 수집
        ...
        # 2. Global Union BBox 계산 (모든 움직임을 포함하는 가장 큰 박스)
        if all_x1:
            g_x1, g_y1 = min(all_x1), min(all_y1)
            g_x2, g_y2 = max(all_x2), max(all_y2)
            
            # 3. Global BBox를 1.2배 확장
            bw, bh = g_x2 - g_x1, g_y2 - g_y1
            cx, cy = g_x1 + bw / 2, g_y1 + bh / 2
            nw, nh = bw * 1.2, bh * 1.2
            
            # 고정된 Crop 좌표 계산 (이미지 경계 처리는 get_item에서 함)
            # 클래스 멤버변수로 저장해둠
            self.global_crop_coords = (int(cx - nw / 2), int(cy - nh / 2), int(cx + nw / 2), int(cy + nh / 2))
            
            print(f"🔒 Global Crop Fixed: {self.global_crop_coords} (W:{nw:.1f}, H:{nh:.1f})")
        else:
            # 데이터가 없을 경우 기본값
            self.global_crop_coords = (0, 0, 1024, 768)
    ...
    def __getitem__(self, idx):
        ...
        # 🔴 [수정] 개별 bbox 대신 고정된 global_crop_coords 사용
        gx1, gy1, gx2, gy2 = self.global_crop_coords
        
        # Clipping (이미지 밖으로 나가지 않게 자름)
        nx1, ny1 = max(0, gx1), max(0, gy1)
        nx2, ny2 = min(w, gx2), min(h, gy2)
        
        # Crop & Resize
        # 만약 고정 박스가 이미지 범위를 완전히 벗어난 경우 예외 처리
...
def run_sapiens_lite_inference(frame_dir, sam_dir, output_dir, config_path, ckpt_path, batch_size=25):
    ...
    # 🔴 [추가됨] 총 BBox 개수 계산 및 출력
    total_bboxes = sum(len(objs) for _, _, objs in tasks)
    print(f"\n📊 [통계 확인]")
    print(f"   - 대상 프레임 수: {len(tasks)}장")
    print(f"   - 총 객체(BBox) 수: {total_bboxes}개 (Inference 횟수)")
    print("-" * 30 + "\n")
    print("Sapiens 모델 로드")
...
if __name__ == "__main__":
    ...
    for target in range(0,1):
        ...
        OUTPUT_DIR = DATA_DIR / "test" / COMMON_PATH / "v4.2_133kpt_lite_0.6b_2"
